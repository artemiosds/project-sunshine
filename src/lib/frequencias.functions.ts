...
const PAYLOAD_FIELDS = [
  "dias_trabalhados","faltas_justificadas","faltas_injustificadas","ferias","licencas",
  "afastamentos","horas_extras","plantoes_extras","adicional_noturno","atestado",
  "he_50","he_100","sobreaviso","incentivo","licenca_premio",
...
export const salvarLinhasFrequencia = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const toInsert = dirty.filter((l) => l._new).map((l) => {
    ...
    for (const l of toUpdate) {
      ...
      const { error } = await supabase.from("frequencia_profissional")
    ...
    const { error: fErr } = await supabase.from("frequencias")
...
const StatusEnum = z.enum([
  "rascunho","enviada","em_analise","com_pendencias","aprovada","rejeitada","arquivada",
]);
...
export const alterarStatusFrequencia = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: z.infer<typeof AlterarStatusSchema>) => AlterarStatusSchema.parse(d))
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    ...
    const { data: freq, error: fErr } = await supabase
      .from("frequencias")
      .select("id, status, competencia_unidade_id, competencia_unidades(competencia_id, competencias(prazo_envio))")
      .eq("id", data.frequencia_id).maybeSingle();
    if (fErr) throw new Error(fErr.message);
    ...
    if (data.status === "enviada") { patch.enviada_por = userId; patch.data_envio = new Date().toISOString(); }
    if (data.status === "aprovada") { patch.aprovada_por = userId; patch.data_aprovacao = new Date().toISOString(); }

    const { error: upErr } = await supabase.from("frequencias")
      .update(patch as never).eq("id", data.frequencia_id);
    if (upErr) throw new Error(upErr.message);

    const label = ACAO_LABEL[data.status];
    if (label) {
      const prazoEnvio = (freq as any).competencia_unidades?.competencias?.prazo_envio;
      const foraPrazo = data.status === "enviada" && !!prazoEnvio && new Date() > new Date(prazoEnvio + "T23:59:59");
    ...
    if (tipoEvento) {
      await emitEvento(supabase, tipoEvento, "frequencia", data.frequencia_id, {
        status_anterior: anterior, status_novo: data.status,
      });
    }
    return { ok: true };
  });
...
export const abrirPendenciaLinha = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: z.infer<typeof PendenciaSchema>) => PendenciaSchema.parse(d))
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    ...
    const { data: freq } = await supabase.from("frequencias")
      .select("status").eq("id", data.frequencia_id).maybeSingle();
    ...
    if (st === "enviada" || st === "em_analise") {
      await supabase.from("frequencias")
        ...
        .eq("id", data.frequencia_id);
      await emitEvento(supabase, EVENTOS.FREQUENCIA_COM_PENDENCIAS, "frequencia", data.frequencia_id, {
        motivo: "pendencia_aberta",
      });
    }
    await emitEvento(supabase, EVENTOS.PENDENCIA_CRIADA, "pendencia", (inserted as any)?.id ?? null, {
      frequencia_id: data.frequencia_id,
      frequencia_profissional_id: data.frequencia_profissional_id,
      titulo: data.titulo,
    });
...
export const inserirLinhasAuto = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: z.infer<typeof AutoInsertSchema>) => AutoInsertSchema.parse(d))
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    await ensurePermission(supabase, userId, ACOES.FREQUENCIA_EDITAR);
...
export const registrarAnexoLinha = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: z.infer<typeof AnexoSchema>) => AnexoSchema.parse(d))
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
